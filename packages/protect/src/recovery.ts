...
) => Promise<{ success: boolean; refundTxId?: string; error?: string }>;
...
export class RecoveryEngine {
  ...
  private readonly actions: Map<string, RecoveryAction> = new Map();

  /** Index: disputeId -> recoveryId */
  private readonly byDispute: Map<DisputeId, string> = new Map();
  ...
  private readonly disputes: DisputeManager;
  ...
  initiate(disputeId: DisputeId): RecoveryAction {
    ...
    if (dispute.status !== 'resolved_refunded' && dispute.status !== 'resolved_partial') {
      throw new Error(
        `Cannot initiate recovery for dispute ${disputeId} with status "${dispute.status}". ` +
    ...
    if (existingId) {
      ...
      if (existing && existing.status !== 'failed' && existing.status !== 'cancelled') {
        throw new Error(
          `Active recovery ${existingId} already exists for dispute ${disputeId}`
        );
      }
    }
    ...
    const recoveryType: RecoveryType =
      dispute.status === 'resolved_partial' ? 'partial_refund' : 'full_refund';
    ...
    this.logger?.info(
      `[RecoveryEngine] Initiated ${recoveryType} of $${amount} for dispute ${disputeId}`
    );
   ...
   */
  async processQueue(): Promise<RecoveryAction[]> {
    const processed: RecoveryAction[] = [];

    while (this.queue.length > 0) {
      const actionId = this.queue.shift()!;
      const action = this.actions.get(actionId);
      if (!action || action.status === 'cancelled') continue;
  ...
  cancel(recoveryId: string): RecoveryAction {
    ...
    if (action.status !== 'pending') {
      throw new Error(
        `Cannot cancel recovery ${recoveryId} with status "${action.status}"`
      );
    }
  ...
  private async executeWithRetry(action: RecoveryAction): Promise<RecoveryAction> {
    action.status = 'processing';
    action.updatedAt = new Date().toISOString();

    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      this.logger?.info(
        `[RecoveryEngine] Executing recovery ${action.id}, attempt ${attempt}/${this.maxRetries}`
      );
      ...
      try {
        ...
        if (result.success) {
          ...
          this.logger?.info(
            `[RecoveryEngine] Recovery ${action.id} completed: refundTxId=${result.refundTxId}`
          );
    ...
    this.logger?.error(
      `[RecoveryEngine] Recovery ${action.id} failed after ${this.maxRetries} attempts: ${action.error}`
    );
