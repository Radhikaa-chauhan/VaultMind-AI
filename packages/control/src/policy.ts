...
import { matchesGlob } from '@vaultmind/core';

/** Tracks cumulative spend per time window bucket */
interface SpendBucket {
  /** Bucket key (e.g., '2026-02-06' for daily) */
  readonly key: string;
...
export class PolicyEngine {
  ...
  private readonly policies: Map<string, SpendPolicy> = new Map();

  /** Spend tracking: 'budget_key:window_key' -> SpendBucket */
  private readonly spendBuckets: Map<string, SpendBucket> = new Map();

  /** Last transaction timestamp per agent (for cooldown) */
  private readonly lastTransactionTime: Map<string, number> = new Map();
  ...
  recordTransaction(tx: AgentTransaction): void {
    for (const policy of this.policies.values()) {
      ...
      for (const budget of policy.budgets) {
        if (budget.currency && budget.currency !== tx.currency) continue;
        if (budget.agentIds?.length && !budget.agentIds.includes(tx.agentId)) continue;
        if (budget.serviceIds?.length && (!tx.service || !budget.serviceIds.includes(tx.service))) continue;
  ...
  private evaluateBudgets(tx: AgentTransaction, policy: SpendPolicy): PolicyEvaluation | null {
    for (const budget of policy.budgets) {
  ...
  private evaluateCooldown(tx: AgentTransaction, policy: SpendPolicy): PolicyEvaluation | null {
    if (!policy.cooldownMs) return null;
  ...
  private evaluateRules(tx: AgentTransaction, policy: SpendPolicy): PolicyEvaluation {
   ...
   */
  private matchesConditions(tx: AgentTransaction, rule: PolicyRule): boolean {
    ...
    // Amount range
    if (cond.minAmount !== undefined && tx.amount < cond.minAmount) return false;
    if (cond.maxAmount !== undefined && tx.amount > cond.maxAmount) return false;
    ...
    if (cond.metadata) {
      for (const [key, value] of Object.entries(cond.metadata)) {
        if (tx.metadata[key] !== value) return false;
      }
    }
  ...
  private readonly actionSeverity: Record<PolicyAction, number> = {
    deny: 0,
    require_approval: 1,
    flag: 2,
    allow: 3,
  ...
  private getMostRestrictive(results: PolicyEvaluation[]): PolicyEvaluation {
    return results.sort(
      (a, b) => this.actionSeverity[a.action] - this.actionSeverity[b.action]
    )[0]!;
  }
  ...
  private getBudgetScope(budget: BudgetLimit): string {
    const parts: string[] = [];
    if (budget.agentIds?.length) parts.push(`agents:${budget.agentIds.join(',')}`);
    if (budget.serviceIds?.length) parts.push(`services:${budget.serviceIds.join(',')}`);
    if (budget.currency) parts.push(`currency:${budget.currency}`);
    return parts.length > 0 ? parts.join('|') : 'global';
  }

  private getWindowKey(window: BudgetLimit['window'], date: Date): string {
    switch (window) {
